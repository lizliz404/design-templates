...
- **Texture source (verified)**: fetch real paper/noise/grain textures (static webp) from [Paper Shaders](https://shaders.paper.design/) — zero-dependency shader library (paper-texture / perlin / simplex / neuro-noise / grain-gradient). Use as low-opacity **multiply** overlay layers; verified ceiling on cream-paper decks is **paper 0.085 / noise 0.05** (safe band ≈ paper 0.03–0.085, noise 0.02–0.05). **Animate the layer, not the file** (`tex-drift` 90s + `tex-grain` 3.4s, `prefers-reduced-motion` → static). Pair with fixed-stage chrome: paper-backed chapter indicator (deep ink, no mid-token `word-break`) + optional 3px progress ruler. Full copy-paste CSS in `animation-patterns.md`.
