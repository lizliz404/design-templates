...
- **Icon system craft** → [`icon-system-craft.md`](./icon-system-craft.md)：Track A product chrome 的线重、尺寸与概念表；稳定生产线与 authored exceptions 分工；不是 favicon / wordmark / OG。
- **Runeicons authoring workbench** → [`runeicons-authoring-workbench.md`](./runeicons-authoring-workbench.md)：与 Iconoir 同为一等图标决策候选；当前只作 Apache-2.0 路径编辑、五 style 与 SVG/JSX 动效导出工作台，未发布 packages 不进 runtime。
...
- **Amicro motion registry intake** → [`amicro-motion-registry-intake.md`](./amicro-motion-registry-intake.md)：162-item MIT React registry 的按件筛选闸；只填真实状态缺口，不装整套、不抄 showcase motion。
- **Proof-first launch storyboard** → [`proof-first-launch-storyboard.md`](./proof-first-launch-storyboard.md)：proof-first hook、关系装配、后置 tension、单句 payoff 的产品片四拍。
- **Progressive 3D assembly** → [`progressive-assembly-3d.md`](./progressive-assembly-3d.md)：一动作一块、k/N、pause/rotate/zoom 和 2D fallback；把 3D 变成状态解释器而非装饰球。
